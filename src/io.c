...
/* sip() 
   读取当前Current 文件，如果文件是一个二进制文件，
   并且SKIP_TEST不是0，则返回非0 */
   
static bool sip (struct file_data *current, bool skip_test)
{
...
static void
slurp_memery (struct file_data *current,char *buff)
{
  size_t cc;

  size_t file_size = strlen(buff);
      cc = file_size + 2 * sizeof (word) - file_size % sizeof (word);
  
  current->bufsize = strlen(buff)+1;
//current->buffer = xrealloc (current->buffer, current->bufsize);
//strcpy(current->buffer,buff);
	  current->buffer = (word *)buff;
	  current->buffered = strlen(buff);
...
static void find_and_hash_each_line (struct file_data *current)
{
  ...
  /*caoyang changed here */
  bool diff_length_compare_anyway = ignore_white_space;
  ...
  while ((char const *) p < suffix_begin)
    {
      ...
      /* caoyang optimize the process here ....*/

	  if (ignore_white_space)
	  {
	  	trace3("--only hash the non-space charactor--\n");
    	while ((c = *p++) != '\n')
     	 	if (! isspace (c))
		h = HASH (h, c);
	  }
...
static void find_identical_ends_memery (struct file_data filevec[],char *buffer_new,char *buffer_old)
{
  word *w0, *w1;
  char *p0, *p1, *buffer0, *buffer1;
  char const *end0, *beg0;
  ...
  //slurp (&filevec[0]);
  printf("here4");
    slurp_memery (&filevec[0],buffer_new);
  prepare_text (&filevec[0]);
  if (filevec[0].desc != filevec[1].desc)
    {
      //slurp (&filevec[1]);
	   slurp_memery (&filevec[1],buffer_old);
      prepare_text (&filevec[1]);
    }
  else
    {
...
printf("job done find_identical_ends\n");
}
...
bool read_files (struct file_data filevec[])
{
  int i;
  bool skip_test = 0 ;
  bool appears_binary =  sip (&filevec[0], skip_test);
  if (filevec[0].desc != filevec[1].desc)
...
bool read_files_memery (struct file_data filevec[],char *buffer_new,char *buffer_old)
{
  int i;
  bool skip_test = 0 ;
  //bool appears_binary =  sip (&filevec[0], skip_test);
  ...
  //find_identical_ends (filevec);
 printf("here3");
  find_identical_ends_memery (filevec,buffer_new,buffer_old);
  ...
  free (equivs);
  free (buckets - 1);
  printf("job done read_files_memery\n");
