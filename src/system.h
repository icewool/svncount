...
/* include the additional tracing tools */
#include <trace.h>
...
#ifndef same_file
/*some attributes lose information in win32
  Added by C.YANG 2006 */
#ifdef _DIFFCOUNT_WIN32_ 
...
/*
  待比较的代码包往往是通过拷贝复制获得的，
  这样创建时间(st_ctime)是不可能一样的
  另外:由于目前代码管理做的不好，相同文件修改时间也经常不同
  (比如穿过FTP后，或者经过CVS)
  为了提高效率，这里暂时也不比较文件修改时间

  这样主要的比较因素是文件大小，这可能是会存在问题的.

  如果感觉可能有文件名一样，文件大小一致，但是文件可能修改的情况
  请考虑使用force-parse-all参数
  
  不能这么写，容易引起统计的内容部分缺失，且在svncount下，即使开启时间功能，
  文件不一致还是不能被，因此，选择默认关闭该功能
